...
router.post("/claim", authenticateToken, async (req, res) => {
  try {
    ...
    console.log(`🎯 Usuario ${address} solicitando tokens...`);
    ...
    if (hasClaimed) {
      return res.status(400).json({
        success: false,
        error: "Tokens ya reclamados",
        message: "Esta dirección ya ha reclamado sus tokens previamente",
      });
    }
...
router.get("/info", async (req, res) => {
  try {
    ...
    res.json({
      ...
      chainId: 11155111,
      totalUsers: users.length,
      message: "CryptoFaucet - Distribuidor de Tokens de Prueba para Sepolia",
    });
  } catch (error) {
